...
review_trigger: a second replay engine appears, a live deployment asks Netbraid to own policy or identity authority, or a promoted intelligence family cannot preserve source lineage and abstention
---
...
The four Rust packages are not yet published on crates.io. The release workflow
now enforces commit-owned versions, trusted publication, dependency order, and
registry verification, but the first publication still requires a deliberate
new version and one scoped-token bootstrap before trusted publishers can take
over. The existing `netbraid-v0.2.0` tag identifies an earlier commit and must
not be reused for newer package bytes.
...
## Phase 1: publish the Rust packages under a new immutable identity

Choose the next release version in the same change that publishes it. From a
clean clone of the intended release commit, bootstrap each crate name with a
fresh scoped token, configure the GitHub trusted publisher, revoke the token,
and let the pinned release workflow publish in dependency order. Verify each
registry package's repository, version, VCS SHA, dirty state, and yanked state
...
Reversibility: scenario fixtures are non-default development data and can be
replaced without changing production records.

## Phase 3: support a Linktop-owned incident evidence capsule

The grounded design selects an explicit Linktop recording transaction for the
operator-owned container. An intermittent incident needs a finite, private
handoff containing the
invocation and tool versions, path generations, typed changes, coverage gaps,
explicit active-operation receipts, artifact digests, and final human and
machine summaries. Ordinary Linktop operation must not retain packets or create
this artifact implicitly.

Before implementation, Linktop records one architecture decision that fixes
the container and completion contract. Netbraid supplies versioned embedded
records and deterministic replay where those are reusable; it does not own the
Linktop transaction. The decision must specify interruption behavior,
no-clobber publication, sensitivity labels, sanitization receipts, and which
...
## Phase 4: broaden evidence through differential boundaries

Add one evidence family at a time from a named immutable artifact and operator
question. Near-term candidates are richer acquisition receipts, multi-interface
capture provenance, and a handshake-local TLS or QUIC feature experiment with
contributing frame references. Add one version-pinned differential runner
against a mature specialist before promoting a native extractor.
...
## Phase 5: promote temporal episodes and baselines
...
## Phase 7: cut over optional live acquisition and retire Go
...
completeness, terminal reason, and replayable receipts. Only after that
baseline passes may an opt-in Muxer policy select among the eligible arms.
...
after the required operator outcomes have Rust owners and no live consumer
imports remain. Then move the existing four-package Rust workspace to the
repository root without changing the dependency graph or inventing a new
umbrella crate.

Consumer: an explicitly configured acquisition deployment, not Linktop's
passive default.

Gate: fixed/seeded replay is deterministic; adaptive receipts record exact
propensity and policy state; a failure injection proves rollback with no lost
or duplicate interval; consumer and dependency scans prove the legacy paths
are deletable.

Reversibility: retain the last known-good writer until the new boundary passes;
adaptive policy can be disabled without changing the evidence contract.
...
1. Linktop registry migration: retain the exact Git revision until Linktop
   ...
   them.
2. Linktop incident-capsule format: container, completion, replay, and
   sanitization contract; Netbraid ownership remains limited to embedded
   reusable records.
